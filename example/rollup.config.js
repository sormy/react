...
const babelOptsVendor = JSON.parse(JSON.stringify(babelOpts));  // clone
// replace typescript preset with flow preset because they can't work together
babelOptsVendor.presets =
  babelOptsVendor.presets.map((plugin) => {
    return plugin[0] === '@babel/preset-typescript' ? '@babel/preset-flow' : plugin
  });
...
export default [
  {
    ...
    plugins: [
      ...
      alias({
        resolve: ['.browser.js', '.js', '/index.js'],
        'react': __dirname + '/../packages/react',
        'react-dom': __dirname + '/../packages/react-dom',
        'scheduler':  __dirname + '/../packages/scheduler',
        'prop-types':  __dirname + '/node_modules/prop-types',
        './ReactFiberHostConfig': __dirname + '/../packages/react-dom/src/client/ReactDOMHostConfig',
        'shared/ReactSharedInternals': __dirname + '/../packages/react/src/ReactSharedInternals',
      }),
      resolve(resolveOpts),
      babel(babelOptsVendor),
      commonjs(),
      replace({
        '__PROFILE__': false,
        '__UMD__': false,
